...
// Conjunto de rótulos conhecidos (normalizados) da configuração
function labelSet(config: ImportConfig): Set<string> {
  const s = new Set<string>();
  config.columns.forEach((c) => c.labels.forEach((l) => s.add(norm(l))));
  (config.lookups || []).forEach((l) => l.labels.forEach((x) => s.add(norm(x))));
  return s;
}

// Lê TODAS as abas, detecta automaticamente a linha de cabeçalho (pula títulos/linhas em branco)
// e devolve objetos {header: value} com metadados __row / __sheet.
export function parseWorkbook(buffer: Buffer, config: ImportConfig): Record<string, any>[] {
  const wb = XLSX.read(buffer, { type: "buffer", cellDates: false });
  const labels = labelSet(config);
  const out: Record<string, any>[] = [];

  for (const name of wb.SheetNames) {
    const ws = wb.Sheets[name];
    if (!ws) continue;
    const aoa = XLSX.utils.sheet_to_json(ws, { header: 1, defval: "", raw: true }) as any[][];

    // encontra a linha de cabeçalho: primeira linha (entre as 15 primeiras) com >= 2 colunas reconhecidas
    let headerIdx = -1;
    for (let i = 0; i < Math.min(aoa.length, 15); i++) {
      const row = aoa[i] || [];
      const matches = row.filter((c) => labels.has(norm(c))).length;
      if (matches >= 2) { headerIdx = i; break; }
    }
    if (headerIdx === -1) continue; // aba sem cabeçalho reconhecível — ignora

    const header = (aoa[headerIdx] || []).map((h) => String(h));
    for (let r = headerIdx + 1; r < aoa.length; r++) {
      const row = aoa[r] || [];
      if (!row.some((c) => String(c).trim() !== "")) continue; // pula linhas vazias
      const obj: Record<string, any> = { __row: r + 1, __sheet: name };
      header.forEach((h, ci) => { if (h && h.trim() !== "") obj[h] = row[ci]; });
      out.push(obj);
    }
  }
  return out;
...
export function runImport(config: ImportConfig, buffer: Buffer, userId: number, overrides: Record<string, any> = {}): ImportResult {
  const rows = parseWorkbook(buffer, config);
  ...
  const tx = db.transaction(() => {
    rows.forEach((raw, idx) => {
      const rowNum = raw.__row || idx + 2; // linha real na planilha
      try {
