...
const REGISTRY: Record<string, ImportConfig & { title: string }> = {
  ...
  assets: {
    ...
    columns: [
      { field: "patrimony_number", labels: ["Número do patrimônio", "Numero do patrimonio", "Patrimônio", "Patrimonio", "patrimony_number"] },
      { field: "name", labels: ["Nome do item", "Item", "Nome", "name"], required: true },
      { field: "category", labels: ["Categoria", "category"] },
      { field: "location", labels: ["Localização", "Localizacao", "Local", "location"] },
      ...
      { field: "status", labels: ["Estado", "Status", "Situação", "Situacao"], enumMap: ASSET_STATUS, default: "bom_estado" },
